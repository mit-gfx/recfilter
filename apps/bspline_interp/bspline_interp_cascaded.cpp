...
int main(int argc, char **argv) {
    ...
    for (int in_w=min_w; in_w<=max_w; in_w+=inc_w) {
        ...
        vector<RecFilter> cF;
        {
            ...
            cF = F.cascade_by_dimension();
            cF = F.cascade_by_causality();
            cF[0].split_all_dimensions(tile_width);
            cF[1].split_all_dimensions(tile_width);
        }
        ...
        // cerr << cF[cF.size()-1] << endl;
        cF[cF.size()-1].compile_jit("stmt.html");

        float time = cF[cF.size()-1].profile(iter);
        ...
        if (!nocheck) {
            ...
            Realization out = cF[cF.size()-1].realize();
