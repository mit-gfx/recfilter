...
/*
    Bicubic Bspline filter
    sgreen 8/1/08

    Bicubic Bspline filter created by converting NVIDIA CUDA toolkit
    recursive Gaussian into first order by enabling SIMPLE_FILTER
...
void benchmark(int image_width)
{
    ...
    // warm-up
    gaussianFilterRGBA(d_img, d_result, d_temp, width, height, sigma, order, nthreads, timer, true);
    ...
    {
        dProcessingTime += gaussianFilterRGBA(d_img, d_result, d_temp, width, height, sigma, order, nthreads, timer, true);
    }
